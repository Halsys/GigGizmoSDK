...
describe("RESTModel class", () => {
	describe("Constructor", () => {
		...
		it("Copy Data", () => {
			...
			expect(copy.dateModified).to.deep.equal(
				moment("2018-04-29T12:00:00Z")
			);
		});
	});
	...
	describe("isValidId", () => {
		it("Not string", () =>
			expect(RESTModel.isValidId(123)).to.equal(false));
		it("Invalid id", () =>
			expect(RESTModel.isValidId("INVALID")).to.equal(false));
		it("Valid id", () =>
			expect(RESTModel.isValidId("de4e0cd7dd992d9be2f20b42")).to.equal(
				true
			));
	});
});
