...
/*#__PURE__*/
function (_RESTModel) {
  _inheritsLoose(Band, _RESTModel);

  function Band() {
    return _RESTModel.apply(this, arguments) || this;
  }

  var _proto = Band.prototype;
  ...
  Band.findOne = function findOne(criteria, token) {
    return _RESTModel2.default.findOne(Band, criteria, token, true);
  };

  Band.findMany = function findMany(criteria, token) {
    return _RESTModel2.default.findMany(Band, criteria, token, true);
  };

  Band.findById = function findById(id, token) {
    return _RESTModel2.default.findById(Band, id, token, true);
  };

  Band.getAllOwned = function getAllOwned(token) {
